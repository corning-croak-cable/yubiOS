...
Latest VM e2e evidence: [run 29702569137 / job 88234024219](https://github.com/yubi-OS/yubiOS/actions/runs/29702569137/job/88234024219), following [refs/vm-e2e-run-29525332901.md](refs/vm-e2e-run-29525332901.md) and SSH follow-up run [29543974333 / job 87776913919](https://github.com/yubi-OS/yubiOS/actions/runs/29543974333/job/87776913919).
...
- [ ] Validate the bcvk root SSH key path on a fresh VM e2e run after the run-29702569137 fixes. The shared test helper now uses bcvk's in-container SSH transport instead of the nonexistent top-level `bcvk ssh` command, while the workflow preserves bcvk's documented `tmpfiles.extra` credential path.
...
- Run 29702569137 reached system targets, started sshd/networking, and proved the forwarded SSH connection reached the guest. The offered key was rejected after CI rewrote bcvk's `tmpfiles.extra` credential, while the harness's `bcvk ssh` probes failed immediately because that top-level command does not exist; current `main` fixes both harness paths pending a fresh VM run.
