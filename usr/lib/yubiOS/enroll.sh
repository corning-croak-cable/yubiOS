...
#
# ```json
# L3079 -- usr/lib/yubiOS/enroll.sh
  hypothesis:  config usr/lib/yubiOS/enroll.sh: NSS 7-relation composition taxonomy (contains / imports / calls / publishes / subscribes / reads / writes / deploys-with / depends-on) -- file declares its in-graph and out-graph surface explicitly
...
#
# Callers: usr/lib/systemd/system/yubiOS-enroll.service (ExecStart=); operator manual invocation.
# Callees: every enroll-{pam,ssh,luks,...}-wrapper.sh (dispatcher); sibling: usr/lib/yubiOS/lib.sh (shared helpers).
#
