...
Gap D status after the canonical sweep: seed replication is done (11/15 clear the bar — effect real, magnitude seed-dependent). Remaining next steps: an amplitude sweep s ∈ {0.25...2} to trace where H1 stops clearing, a larger optimizer budget to test whether the 4 sub-bar seeds are optimizer misses, and running the optimized φθ against the real corpus behind its matched null.
