...
Appendix D of learned-latent-curves-2026-08-06.tex. A NEW, more rigorous
version of the synthetic-manifold benchmark that actually tests the
inductive-bias claim of the hyperspherical-harmonic variant.

DIFFERENCES FROM PR #192 (the prior synthetic-manifold-benchmark-2026-08-06.py):
  ...
  Other rigor upgrades:
    ...
    - Paired t-test on per-seed DELTA = sphere - flat (significance
      threshold p < 0.05).
    - Capacity matched by basis COUNT (16 vs 9 effective - the prior
      16 vs 16 was capacity-confounded; the 16 Fourier basis has 7
      zero columns from sin(0 * theta) and the (i=0, j=0) tensor
      products).
...
Prediction (must hold under the benchmark's own test):
  - T^2 -> FLAT wins (sphere is wrong prior).
  - S^2 -> SPHERE wins (sphere is right prior).
  - If sphere wins on BOTH, the inductive-bias claim is FALSIFIED.

Outputs (in /var/workspace/session/subagents/):
  - manifold-coord-benchmark-results.json  (per-seed + summary + p-values)
  - chart-manifold-coord-2026-08-06.png     (grouped bar chart with error bars)
"""
...
def render_chart(results: dict, out_path: str) -> None:
    ...
    d.text((W / 2, 22),
           "Manifold-Coord Benchmark - Inductive-Bias Test (v2)",
    ...
    for gi, (label, mkey) in enumerate(zip(manifolds, manifold_keys)):
        ...
        d.rectangle([x, y_top, x + bar_w, y_zero], fill=colors["sphere"])
        ...
        x = g_left + bar_w + gap
        y_top = to_y(f_mean)
        d.rectangle([x, y_top, x + bar_w, y_zero], fill=colors["flat"])
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        out_dir = Path("/var/workspace/documents/github-yubios-KS9n5GAT/subagents/manifold-coord-benchmark-2026-08-06")
    out_dir.mkdir(parents=True, exist_ok=True)

    print("=== manifold-coord-benchmark-2026-08-06 (NEW, rigorous) ===")
    ...
    json_path = out_dir / "manifold-coord-benchmark-results.json"
    payload = {
        "benchmark": "manifold-coord-benchmark-2026-08-06",
        "version": "v2 - designed after advisor critique of PR #192",
    ...
    chart_path = out_dir / "chart-manifold-coord-2026-08-06.png"
