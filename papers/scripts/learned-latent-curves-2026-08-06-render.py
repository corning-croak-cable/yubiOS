...
story.append(Paragraph(
    ...
    "The curve maps show the geometry the primitive basis induces, the drift figures show how that "
    "geometry moves between corpora and between cycles, and the per-dimension fit plots in E.8 "
    "record the PC1+PC2 concentration gate at every basis dimension tested. "
...
story.append(Paragraph(
    "The 2D stereographic projection of <i>S</i><sup>2</sup> for the full yubiOS corpus: every file "
    "is mapped to a point via 9-D primitive coverage &rarr; PCA top-2 &rarr; stereographic lift &rarr; "
    "M&ouml;bius reparameterization. Dense regions are corpus parts that share a primitive-closure "
    "pattern, and the fitted curve is the one-dimensional path the RSI loop walks when it schedules "
    "work. The cycle-3 recomputation below it shows the map is stable under a corpus that grew by an "
    "order of magnitude.",
    style_body,
))
...
if Path(_ecm1).exists():
    ...
    story.append(Paragraph(
        "<b>Figure E.1.</b> Full corpus curve map (2D stereographic projection of "
...
if Path(_ecm1b).exists():
    ...
    story.append(Paragraph(
        "<b>Figure E.2.</b> Full corpus curve map after the cycle-3 refs ingest (2026-08-07). "
        "Same pipeline as Figure&nbsp;E.1; gross curve shape is preserved under a ~10&times; corpus "
        "growth, evidencing corpus-scale stability rather than a small-sample artifact.",
        style_caption,
    ))
story.append(Spacer(1, 6))

# E.2 384-dimensional curve map
story.append(Paragraph("E.2&nbsp;&nbsp;384-dimensional curve map", style_h2))
story.append(Paragraph(
    "Replaces the 9-D binary primitive basis with the Fibonacci-sphere "
    "<i>Y</i><sub>3</sub><sup>3</sup> construction at 384 symmetric azimuthal lobes "
    "(&#8467; = 384, <i>m</i> = 3) over 2610 items. The manifold structure survives the basis change "
    "while the density gradient sharpens. This is the variant that reaches PC1+PC2 = 1.0000 in "
    "Table&nbsp;E.1, and it is the strongest evidence that concentration is driven by the spherical "
...
if Path(_ecm2).exists():
    ...
    story.append(Paragraph(
        "<b>Figure E.3.</b> 384-dimensional curve map (Fibonacci-sphere "
        "<i>Y</i><sub>3</sub><sup>3</sup> basis, 384 symmetric azimuthal lobes, 2610 items, "
        "2026-08-07). PC1+PC2 = 1.0000; see Table&nbsp;E.1.",
        style_caption,
    ))
story.append(Spacer(1, 6))

# E.3 Multi-corpus curve map
story.append(Paragraph("E.3&nbsp;&nbsp;Multi-corpus curve map", style_h2))
story.append(Paragraph(
    "The same <i>S</i><sup>2</sup> projection coloured by source corpus "
    "(<font face='Courier'>skills/</font>, <font face='Courier'>docs/</font>, "
    "<font face='Courier'>refs/</font>, <font face='Courier'>self/</font>) across the 2286 sections "
    "used for the 9-D and 16-D fits. The corpora occupy overlapping but distinguishable arcs, which "
    "is the precondition for the drift detector in E.4&ndash;E.6: alignment is only meaningful if the "
...
if Path(_ecm3).exists():
    ...
    story.append(Paragraph(
        "<b>Figure E.4.</b> Multi-corpus curve map (2026-08-07) &mdash; 2286 sections across four "
        "corpora, coloured by source. Overlapping-but-separable arcs make cross-corpus alignment "
        "meaningful.",
        style_caption,
    ))
story.append(Spacer(1, 6))

# E.4 Drift: aligned curves
story.append(Paragraph("E.4&nbsp;&nbsp;Drift detection: aligned curves", style_h2))
story.append(Paragraph(
    "Aligns the fitted curves of two corpora after M&ouml;bius reparameterization and marks the warp "
    "regions where the geodesic distance between aligned points exceeds the per-cycle tolerance. "
    "Warp regions are the operational trigger for the self-archaeology cadence: they name the arc of "
    "the corpus whose primitive coverage has moved since the last cycle, which is exactly the "
...
if Path(_drift).exists():
    ...
    story.append(Paragraph(
        "<b>Figure E.5.</b> Aligned curves across corpora after M&ouml;bius reparameterization. "
        "Shaded warp regions mark arcs whose aligned-point geodesic distance exceeds the per-cycle "
        "tolerance; these are the arcs the RSI loop schedules first.",
        style_caption,
    ))
story.append(Spacer(1, 6))

# E.5 Drift: aligned points
story.append(Paragraph("E.5&nbsp;&nbsp;Drift detection: aligned points", style_h2))
story.append(Paragraph(
    "The point-level view underneath Figure&nbsp;E.5: each corpus item is plotted at its aligned "
    "curve parameter, so a warp region appears as a local divergence of the two point clouds rather "
    "than only as a gap between two smoothed curves. It is the diagnostic used to confirm that a "
...
if Path(_points).exists():
    ...
    story.append(Paragraph(
        "<b>Figure E.6.</b> Aligned-points scatter underlying Figure&nbsp;E.5. Per-item alignment "
        "residuals distinguish a coherent file-group warp from a single-outlier artifact.",
        style_caption,
    ))
story.append(Spacer(1, 6))

# E.6 Drift: series-aligned curves
story.append(Paragraph("E.6&nbsp;&nbsp;Drift detection: series-aligned curves", style_h2))
story.append(Paragraph(
    "Aligns the fitted curves across the five basis dimensions of the series run (7-D, 9-D, 16-D, "
    "24-D, 384-D) rather than across corpora. Because the corpus is held fixed and only the basis "
    "changes, divergence between the aligned curves isolates the effect of the basis itself &mdash; "
    "and the 24-D curve is visibly the one that departs, the same failure Table&nbsp;E.1 records "
...
if Path(_series).exists():
    ...
    story.append(Paragraph(
        "<b>Figure E.7.</b> Series-aligned curves across the five basis dimensions on a fixed corpus. "
        "With the corpus held constant, curve divergence isolates the basis effect; the 24-D arm is "
        "the outlier.",
        style_caption,
    ))
story.append(Spacer(1, 6))
...
# E.7 Keystone
story.append(Paragraph("E.7&nbsp;&nbsp;Keystone: five-dimension gate summary", style_h2))
story.append(Paragraph(
    "The keystone diagram: all five basis dimensions overlaid with their primitive guides and their "
    "PC1+PC2 gate status in one frame. It is the single figure that carries this appendix's claim "
    "&mdash; four of five dimensions clear the &ge; 0.40 concentration gate, and the one that fails "
    "(24-D) fails because the feature space was widened past the 9-D primitive basis with 12 NSS and "
    "3 meta features, not because the spherical construction broke. Read it together with "
...
if Path(_keystone).exists():
    ...
    story.append(Paragraph(
        "<b>Figure E.8. Keystone.</b> All five basis dimensions (7-D, 9-D, 16-D, 24-D, 384-D) with "
        "primitive guides and PC1+PC2 gate status. Four of five clear the &ge; 0.40 gate; 24-D fails "
        "at 0.2993 &mdash; the limit of the inductive bias once the feature space is extended beyond "
        "the 9-D primitive basis.",
        style_caption,
    ))
story.append(Spacer(1, 6))

# E.8 Per-dimension fit plots + gate table
story.append(Paragraph("E.8&nbsp;&nbsp;Per-dimension fit plots and the concentration gate", style_h2))
story.append(Paragraph(
    "Table&nbsp;E.1 reports the PC1+PC2 concentration for each basis dimension against the &ge; 0.40 "
    "gate, and Figures&nbsp;E.9&ndash;E.13 show the corresponding per-dimension fit plots. The five "
    "dimensions are not five independent experiments &mdash; they are a nested sweep of the input "
    "representation on (nearly) the same corpus.",
    style_body,
))
_gate_data = [
    [Paragraph("<b>Dim.</b>", style_cell_hdr),
     Paragraph("<b>Corpus</b>", style_cell_hdr),
     Paragraph("<b>N</b>", style_cell_hdr),
     Paragraph("<b>PC1+PC2</b>", style_cell_hdr),
     Paragraph("<b>Gate</b>", style_cell_hdr),
     Paragraph("<b>Basis</b>", style_cell_hdr)],
    [Paragraph("<b>7-D</b>", style_cell_left),
     Paragraph("refs (7-D basis)", style_cell_left),
     Paragraph("1862", style_cell_body),
     Paragraph("1.0000", style_cell_body),
     Paragraph("PASS", style_cell_body),
     Paragraph("SH L=3, 16 basis fns", style_cell_left)],
    [Paragraph("<b>9-D</b>", style_cell_left),
     Paragraph("4 corpora (9-D basis)", style_cell_left),
     Paragraph("2286", style_cell_body),
     Paragraph("0.4565", style_cell_body),
     Paragraph("PASS", style_cell_body),
     Paragraph("SH L=3, 16 basis fns", style_cell_left)],
    [Paragraph("<b>16-D</b>", style_cell_left),
     Paragraph("4 corpora (16 SH fns)", style_cell_left),
     Paragraph("2286", style_cell_body),
     Paragraph("0.4627", style_cell_body),
     Paragraph("PASS", style_cell_body),
     Paragraph("SH L=3, 16 real SH on S2", style_cell_left)],
    [Paragraph("<b>24-D</b>", style_cell_left),
     Paragraph("4 corpora (9-D + 12 NSS + 3 meta)", style_cell_left),
     Paragraph("2286", style_cell_body),
     Paragraph("0.2993", style_cell_body),
     Paragraph("<b>FAIL</b>", style_cell_body),
     Paragraph("SH L=3, 16 fns (extended feat.)", style_cell_left)],
    [Paragraph("<b>384-D</b>", style_cell_left),
     Paragraph("4 corpora (Fibonacci-sphere)", style_cell_left),
     Paragraph("2610", style_cell_body),
     Paragraph("1.0000", style_cell_body),
     Paragraph("PASS", style_cell_body),
     Paragraph("Y3^3 basis, 384 lobes (l=384,m=3)", style_cell_left)],
]
_gate_tbl = Table(_gate_data, colWidths=[0.5*inch, 1.6*inch, 0.5*inch, 0.65*inch, 0.55*inch, 2.7*inch])
_gate_tbl.setStyle(TableStyle([
    ("BACKGROUND", (0, 0), (-1, 0), HexColor("#1a3a5c")),
    ("TEXTCOLOR", (0, 0), (-1, 0), HexColor("#ffffff")),
    ("FONTSIZE", (0, 0), (-1, -1), 9),
    ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
    ("GRID", (0, 0), (-1, -1), 0.5, HexColor("#888888")),
    ("BACKGROUND", (0, 4), (-1, 4), HexColor("#f6e9e9")),
    ("LEFTPADDING", (0, 0), (-1, -1), 5),
    ("RIGHTPADDING", (0, 0), (-1, -1), 5),
    ("TOPPADDING", (0, 0), (-1, -1), 3),
    ("BOTTOMPADDING", (0, 0), (-1, -1), 3),
]))
story.append(_gate_tbl)
story.append(Spacer(1, 4))
story.append(Paragraph(
    "<b>Table E.1.</b> Five-dimension concentration gate. PC1+PC2 is the fraction of variance carried "
    "by the top two principal components of the primitive-coverage matrix; the gate is &ge; 0.40. "
    "7-D passes trivially (7-D &sub; 9-D, single corpus). 9-D is the headline dimension; 16-D is "
    "statistically indistinguishable. 24-D fails &mdash; concentration is a property of the primitive "
    "basis and does not survive arbitrary feature-space widening. 384-D is the strongest concentration "
    "in the sweep.",
    style_caption,
))
story.append(Spacer(1, 8))
...
for _fig, _dim, _fp, _pc, _gate, _blurb in _fit_specs:
    if Path(_fp).exists():
        story.append(Image(_fp, width=4.8*inch, height=3.6*inch))
        story.append(Paragraph(
            f"<b>Figure {_fig}.</b> {_dim} fit plot &mdash; PC1+PC2 = {_pc}, gate {_gate}. {_blurb}",
            style_caption,
        ))
story.append(Spacer(1, 6))

# E.9 N-D PCA viewer
story.append(Paragraph("E.9&nbsp;&nbsp;N-dimensional PCA viewer", style_h2))
story.append(Paragraph(
    "The static export of the interactive N-dimensional PCA viewer at the cycle-3 refs snapshot. It "
    "is the projection used for the headline ablation in Section&nbsp;6 and the tool by which the "
    "24-D gate failure was first observed: the extended feature space spreads variance across more "
...
if Path(_nd).exists():
    ...
    story.append(Paragraph(
        "<b>Figure E.14.</b> N-dimensional PCA static export (cycle-3 refs, 2026-08-07). Static "
        "counterpart to the interactive HTML viewer; the same projection backs the ablation in "
        "Section&nbsp;6.",
        style_caption,
    ))
