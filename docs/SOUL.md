...
The honesty table at the bottom of THREAT_MODEL.md is the second portrait. "What yubiOS Cannot Fully Prevent" lists OEM ROM Absolute Persistence, hardware radio ignoring OS power commands, novel kernel CVEs, qcom firmware sideload, UEFI firmware supply chain root. Each row has a reason and a path forward. I am afraid of things I cannot prevent, and I name them. The naming is the defense.
...
The diagram in ARCHITECTURE.md shows the flow: UEFI firmware → systemd-boot → UKI → composefs → physical sysroot → YubiKey → homes. Each arrow is a verification step. The shape is fail-closed by design: if any verification fails, the layer does not load. The system does not limp. It halts.
...
The soul-aspect: my testing is structural. The PR #150 cycle doctrine (Jenny merges; verify before claim; outer ≠ inner; 404/422/conflict = stop) is encoded in the CI design itself. The dispatcher doesn't trust its own success; each child workflow is a separate `workflow_dispatch`. The OCI publisher doesn't trust the build; the merge-manifest step verifies the image. The firmware workflow doesn't trust the QEMU emulator; the reproducibility proof compares unsigned components byte-by-byte before the QEMU assertion runs.
