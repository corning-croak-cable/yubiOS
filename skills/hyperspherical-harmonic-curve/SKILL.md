...
The variant was fit through four RSI cycles (cycle-3 fixpoint + cycle-4 carryover closure). **All calibration gates measured PASS at both phases. Matched-parameter ablation FIRES** at both phases and both basis implementations. The ε_basis unit test bug (comparing G to I instead of G/(1/4π) to I) was found and corrected — the basis IS orthonormal. Möbius refinement converged and preserves cross-ratio within 3.08e-07. Spectral-mass gate ρ, high-degree mass, sparse-cell counts all measured and PASS. **Prior-art depth-verified (cycle 4)**: arXiv 2601.20528 (Spectral Bayesian Regression on the Sphere, Durastanti 2026) covers Fourier-on-S² as Bayesian regression with statistical-theory focus, but **NO corpus audit, NO learned Möbius, NO sparse-cell detection** — the variant's **application-layer novelty is CONFIRMED** (not covered by prior art). OpenReview `g6UqpVislvH` API blocked at 403 (`ChallengeRequiredError` requirement) — **honest gap documented, mechanism-layer novelty remains BORDERLINE on that one unverified hit**. Threshold calibration status: all 9 gates in `## Key Assumptions` have measured PASS values (ρ, high-degree mass, ε_basis, cross-ratio, etc.); the threshold values (0.10, 0.40, 1e-2, 1e-4) are now empirically validated, not principle-only.
