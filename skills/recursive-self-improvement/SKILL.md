...
**Three cycles is the hard limit.** Past that, escalate to the user — the skill's content is being treated as the variable when the variable is the surrounding context (workflow position, audience change, project shift). Stop editing; ask whether the skill itself should be retired or repositioned.
