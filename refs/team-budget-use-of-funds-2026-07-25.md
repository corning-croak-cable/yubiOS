# yubiOS team, budget, and use of funds â Year 1 framework
...
both list "Financial: [Sauna will update this]" â there is no funding amount,
...
framework* OMN-74 also asks for â the part that doesn't require a number no one
has told this session yet â plus a flag on what's needed to fill in the
envelope itself.

## 1. Year 1 budget envelope â open item
...
a funding/grant document lands â OMN-86, or an update to RULES.md's Financial
section), sections 2â5 below can be applied against it with weights, not
...
|---|---|---|
| Engineering (trust-chain work) | Highest | Every commercial offer in [refs/offer-pricing-architecture-2026-07-25.md](offer-pricing-architecture-2026-07-25.md) (OMN-71) is gated on a specific engineering readiness gate (B-REAL-FIDO2, B-VM-CTAP2, PR #32, ARM64 board rehearsal). No offer sells without these closing first â spend here is the precondition for revenue, not a nice-to-have. |
| Legal (naming/trademark, entity) | High, front-loaded | [refs/naming-licensing-provenance-2026-07-25.md](naming-licensing-provenance-2026-07-25.md) (OMN-81) flags the "yubiOS"/Yubico trademark question as high-severity and unresolved; OMN-72's entity decision blocks signing any real contract (support/SLA/consulting offers). This is a front-loaded, not steady-state, cost â resolve it early, don't spread it evenly across the year. |
| Customer/pilot work | Medium, event-driven | Scoped to actual paid-pilot activity per the metrics doc's ([refs/metrics-and-reporting-2026-07-25.md](metrics-and-reporting-2026-07-25.md), OMN-75) "paid-pilot count and outcome" metric â spend here scales with pilots landing, not a fixed headcount assumption. |
| Support | Low initially, triggered | Only becomes a real cost once O3 (support/SLA) actually sells â per OMN-71's revenue-priority ordering, O3 is gated on B-VM-CTAP2 closing, so budget for it activates on that gate, not calendar time. |
| Community | Low, mostly non-monetary | The covenant (OMN-70) and conflict policy (OMN-82) commitments (public trust chain, coordinated disclosure) are process commitments, not headcount â community spend here is closer to "time," which this framework can't price without knowing the eventual team size. |
...
1. **Legal counsel (contractor, not hire) â first.** Section 1's trademark
...
2. **Engineering contractor/hire for the specific open readiness gate** â
   ...
   status. Don't hire generically for "engineering" â hire against the named
   blocker.
3. **First customer-facing hire, triggered by the first signed pilot** â not
   ...
   trigger fired yet.
4. **Support hire, triggered by O3 (SLA) actually selling** â same
...
- **Legal** (trademark, entity formation, contract review) â inherently
...
- **Specific engineering gates** (e.g. an ARM64 board bring-up specialist for
  the fTPM/OP-TEE stack) â scoped, evidence-target-bound work per
...
- **Training delivery** (O6 offer) â can be contracted per-cohort rather than
...
ongoing maintenance (Secure Boot signing infra, LUKS2/FIDO2 code) â MISSION.md's
...
| Role | Trigger |
|---|---|
| Legal contractor | Immediate â section 1/2's front-loaded item, no evidence gate needed to start |
...
- Section 1 (the actual envelope) depends on a real funding/revenue figure
  landing in memory or being provided directly â this document does not
  produce or guess one.
- Sections 2â5 depend on **OMN-71** (pricing, PR #108), **OMN-75** (metrics,
...
This file's lens is **L198** in `lenses.json` (score 44/50, verdict **PARTIAL**, k=8/9). Full experiment: hypothesis `refs/team-budget-use-of-funds-2026-07-25.md covers all 9 primitives in the internal-big-picture basis`, method `9-D primitive binarization (purpose, examples, guidelines, constraints, verifica...`. See root `new-ideas-2026-08-12.md` for the cycle-3 summary.
