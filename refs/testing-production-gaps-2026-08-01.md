# yubiOS testing & production gaps â 2026-08-01
...
| 14 | Group reachability | `docs/CI_MAP.md` group lists | `ci_test-ftpm-tpm0.yml`, `ci_test-fedora-bootc-arm64-pull.yml`, `ci_test-vgpu-vm.yml` in **no** group â `group=all` misses them |
...
**1 â Real-hardware Secure Boot negative-tamper evidence** Â· *Critical* Â· A signed UKI failing `sbverify` could still be published and accepted if SB variable enforcement and PCR handoff are never measured against real firmware. *Mitigation:* QEMU OVMF + swtpm proves only the path that already works. *Cost:* 3â4 weeks + one Jenny sign-off; amd64 VM lane first, arm64 behind `B-ARM64-PATHA`. *Fix:* land `ci_test_sealed-uki-vm.yml` (PR #154 + PR #155), capture 3 green negative-tamper runs against `:dev-<sha>`, add `tests/unit/test-bootc-sealed-lane.bats`.

**2 â ARM64 Path A real-board proof** Â· *Critical* Â· No production claim for ROCK 5B / ROCKPro64; Path B evidence is not equivalent; QEMU proves build shape, not fuses, RPMB, or owner root-of-trust custody. *Cost:* 6â12 weeks + sacrificial board + Jenny for ritual phases. *Fix:* OMN-45 ROTPK rehearsal, OMN-46 OP-TEE/RPMB/fTPM/U-Boot on hw, OMN-47 signed-UKI boot; retain `refs/rock5b-sacrificial-rehearsal-<date>.md`.

**3 â Physical-YubiKey production-confidence run** Â· *High* Â· `B-REAL-FIDO2` holds the trust claim; software proves function, not physical presence, firmware ownership, or RPMB freshness. *Cost:* 1â2 weeks + Jenny for the 12 scenarios. *Fix:* execute OMN-63's 12 scenarios against `0mniteck/yubios:<sha>`; retain `refs/yubikey-hardware-run-<date>.md`.

**4 â Runtime hardening evidence** Â· *High* Â· Static Bats pass; nobody has booted a target image to see `RestrictFileSystems=~@network` fire during enrollment. *Cost:* 1 week once `ci_test-vm.yml` boots a hardened image. *Fix:* post-enrollment assertions on `systemctl show yubiOS-enroll.service -p RestrictFileSystems` and `systemd-analyze security yubiOS-enroll.service`.

**5 â bootc upgrade/rollback + sysext + portable services** Â· *High* Â· A/B counter saturation, `bootc switch --transport=oci` rollback, sysext against immutable `/usr`, portable-service lifecycle â no CI assertion for any. *Cost:* 2 weeks. *Fix:* `tests/vm/test-bootc-upgrade-rollback.sh`, `test-sysext-lifecycle.sh`, `test-portable-service.sh`.

**6 â SBOM + SLSA L3 + cosign** Â· *High* Â· Consumers cannot verify what they pull; mutable `latest`/`dev` have no channel binding; SB key provenance unattached. *Cost:* 1 week. *Fix:* `provenance: true, sbom: true` on `docker/build-push-action` in `yubiOS-ci.yml` + `ci_mkosi-installer.yml`; `cosign sign --yes`; add `tests/vm/test-oci-provenance.sh`.

**7 â GPU trust boundary on real RK3588** Â· *High* Â· ADR-031's ladder is documented, unproven; CI covers virtio-gpu only. *Cost:* 4â6 weeks + sacrificial board. *Fix:* vfio-user creds-locked handoff test in QEMU; IOMMU group isolation on RK3588; CI rejection of `vm-gpu=virtio` when `host-gpu=pci` is available.

**8 â Top-level input-shape doctrine** Â· *Medium* Â· The whole 7-day cluster's shared root. BLOCKERS.md covers lex-sort and `/dev/vfio` retroactively; nothing proactive. *Cost:* 1 week author + 1 week wire. *Fix:* `.github/actions/validate-input-shape/action.yml` asserting (a) inputs are enums, (b) drop-in filenames lex-sort after upstream, (c) OCI refs digest-pinned, (d) channel selection role-bound; invoke from `ci.yml`. `playbooks/` covers the human half now.

**9 â 3 workflows unreachable from any group** Â· *Medium* Â· `group=all` silently misses `ci_test-ftpm-tpm0.yml`, `ci_test-fedora-bootc-arm64-pull.yml`, `ci_test-vgpu-vm.yml`. *Cost:* 0.5 day. *Fix:* Bats test asserting the `group:*` union covers all dispatch-enabled workflows, or add `group=tests-standalone`.

**10 â Fork upstream-sync drift** Â· *Medium* Â· 4 forks lost auto-sync after PR #145; no nag. *Cost:* 1 week. *Fix:* daily `schedules/fork-drift-watch/` firing `group=fetches`; file a Linear issue on a non-empty `PINNED.md` diff.

**11 â Workflow token-scope audit** Â· *Medium* Â· Every `permissions:` block is hand-written; 20/25 lack `concurrency:`; the GH_TK cleanup was the symptom. *Cost:* 2â3 days. *Fix:* `.github/scripts/audit-workflow-permissions.sh` asserting every workflow declares `permissions:` and only the 4 builders accept `Docker_push`.

**12 â Missing negative-path test scripts** Â· *Low/Medium* Â· Green-only coverage on the most security-critical primitives; no negative test that a wrong PKCS#11 URI fails closed. *Cost:* 1â2 weeks. *Fix:* `test-secure-boot-tamper.sh`, `test-oci-tag-channel.sh`, `test-bcvk-passthrough.sh`, `test-policy-rejection.sh`, each with a Bats existence assertion.
...
1. **Land `ci_test_sealed-uki-vm.yml` with 3 negative-tamper assertions** â PR #154 scope: signed UKI via SoftHSM PIV 9c + `provider:pkcs11`, OVMF boot with ROTPK in `db`, then tampered UKI / tampered composefs / unsigned UKI. Parent OMN-53. *Urgent Â· Todo*
2. **Sacrificial RK3588 Path A rehearsal â ROTPK + fTPM + signed UKI boot** â OMN-45/46/47 as sub-issues; closes `B-ARM64-PATHA` and `B-RK3588-TPL`. *Urgent Â· Todo*
3. **Execute 12-scenario physical-YubiKey production-confidence run** â OMN-63's scenarios against `0mniteck/yubios:<sha>`; closes `B-REAL-FIDO2`. *Urgent Â· Todo*
4. **Wire runtime hardening assertions in `ci_test-vm.yml`** â post-enrollment `RestrictFileSystems` + `systemd-analyze security` + `ConditionSecurity=measured-os`; closes `B-HARDENING-RUNTIME`. *High Â· Todo*
5. **Author bootc upgrade/switch + sysext + portable-service VM tests** â three new `tests/vm/` scripts. *High Â· Backlog*
6. **Wire SLSA L3 provenance + SPDX SBOM + cosign in yubiOS-ci + mkosi-installer** â closes the 0/25 SBOM and 2/25 provenance gaps. *High Â· Backlog*
7. **GPU trust boundary proof on real RK3588** â vfio-user handoff, IOMMU isolation, ADR-031 preference enforcement. *High Â· Backlog*
8. **Add top-level input-shape doctrine and CI gate** â `validate-input-shape` action + a BLOCKERS.md Permanent CI-Evidence Patterns entry. *High Â· Backlog*
9. **Assert every `workflow_dispatch` workflow is reachable from a group** â Bats union test, or a `tests-standalone` group. *Medium Â· Backlog*
10. **Daily fork-upstream drift detection schedule** â `group=fetches` daily; file an issue tagged `area:supply-chain` on a non-empty `PINNED.md` diff. *Medium Â· Backlog*
11. **Workflow token-scope audit script** â assert `permissions:` everywhere; `Docker_push` only on the 4 builders; no gratuitous `contents: write`. *Medium Â· Backlog*
12. **Author 4 missing VM test scripts** â negative-tamper, OCI-channel, YubiKey-passthrough, policy-rejection, each + a Bats assertion. *Low Â· Backlog*

## Cross-references

- **Commits:** `21ba013c910f6a2e1fa46646fd755adc0de88f13` (main HEAD); `59f4332` â `f92c6010` (lex-sort); `a1940330` (PR #143, artifact split, closed OMN-51); `8ccffa71`, `d2646452` (digest bumps); `a49e95db` (PR #148); `8b5b20b` (PR #147); `5200f0b`, `5342867`, `6dad3733` (`allow_real_u2f`); `7eba4856e7` (job 91037694742, unsealed BLS proof); `3211e25a617e` (V39 sealed-UKI); `1c284b48826f` (PR #154 lane); `0cb68518bef0` (PR #155 stub fill-in, MERGED); `1d0666d77c0b` (V83 arm64 boot_timeout slack); `95565a0e` (dev-short-sha tag fix); `a50ecac42cc0` (V37 colon-in-step-name parse failure).
...
This file's lens is **L403** in `lenses.json` (score 22/50, verdict **PARTIAL**, k=4/9). Full experiment: hypothesis `refs/testing-production-gaps-2026-08-01.md covers all 9 primitives in the internal-big-picture basis`, method `9-D primitive binarization (purpose, examples, guidelines, constraints, verifica...`. See root `new-ideas-2026-08-12.md` for the cycle-3 summary.
