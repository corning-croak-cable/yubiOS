...
| bootc install | bootc docs state `bootc install` has `to-disk` and `to-filesystem`; as of bootc 1.11, `to-disk` uses DPS partition type GUIDs but still injects `root=UUID=` for compatibility. DPS auto-discovery requires `to-filesystem --root-mount-spec=""` plus a Boot Loader Interface-capable bootloader. | Do not promise root-karg omission for the baseline yubiOS installer path until the bootloader/initramfs path is proven. |
...
- Add bootc 1.11 DPS/root discovery distinction to supply-chain/install tracking.
