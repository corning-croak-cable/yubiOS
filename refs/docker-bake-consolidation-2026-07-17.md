...
| `buildx create --use` selects a builder in the invoking Docker configuration, while Bake's `--builder` flag overrides that configured selection. | Containerized jobs may name their user-scoped builder. Bare-runner artifact jobs follow the current workflow pattern and do not try to select the `hardened` builder created by a separate `sudo` configuration. |
...
| `ci_firmware-rk.yml` | `firmware` | Native/cross firmware compilation, QEMU evidence, artifact download, RK3588 TPL gate, and bare-runner builder selection remain in Actions. |
...
| `ci_mkosi-installer.yml` | `installer` | SoftHSM, `/run`, user namespaces, mkosi, UKI verification, payload preparation, and bare-runner builder selection remain host operations. |
...
- Reconciled the branch through `main` commit `a95f185` and retained the current bare-runner Buildx invocation pattern.
