...
§2's commercial boundaries: everything below sells *around* the trust chain
(services, hardware, support), never access *to* it. No price in this document is
a committed number — every figure is an explicit hypothesis to validate in a paid
...
|---|---|---|---|
| O1: Managed fleet enrollment | Recurring (subscription) | Onboarding + ongoing enrollment/rotation of YubiKeys across a fleet (PIV, FIDO2, LUKS2) via a hosted dashboard | Allowed — dashboard is convenience; `homectl`/`bootc`/API access to the same enrollment state stays unpaywalled per covenant §2 |
| O2: Hardware bundles | Non-recurring | Pre-flashed device + enrolled YubiKey shipped as a unit | Allowed — sells hardware, not trust-chain access |
| O3: Support contracts / SLAs | Recurring | Response-time-bound support for production yubiOS fleets | Allowed — security fixes still ship to everyone simultaneously per covenant §3 Disclosure; SLA buys *response time*, not the fix itself |
| O4: Managed CI / build infrastructure | Recurring | Hosted mkosi/bcvk build pipeline with PIV signing, so customers don't run their own signing infra | Allowed, with care — the customer's own PIV key material must stay customer-held per covenant §2 ("no feature that requires surrendering key material... as a condition of paying") |
...
| O7: Public-security grants/pilots | Non-recurring (external funding) | Paid pilots or grant-funded deployments with public-interest orgs (schools, municipalities, journalists) | Allowed — ties to OMN-86 (public-security funding targets), a separate Linear issue; not designed here |

This catalog deliberately excludes any SKU that gates a security fix, a trust-chain
feature, or requires key custody — those are the covenant §2 "not allowed" list,
...
|---|---|---|
| O1 Managed enrollment | Per-seat or per-device recurring fee, tiered by fleet size | A design partner willing to pay list price for a 10–50 device pilot, without a discount that erases the tier logic |
| O2 Hardware bundles | Cost-plus on top of the ~$25–70 YubiKey unit cost cited in MISSION.md, plus device cost and a margin for pre-flash/enrollment labor | Whether customers prefer buying a bundle vs. buying a bare YubiKey + self-flashing (a real substitution test, not a survey answer) |
...
|---|---|
| O1 Managed enrollment | Requires B-REAL-FIDO2 real-hardware evidence run (per [refs/yubikey-hw-validation-scenarios-2026-07-25.md](yubikey-hw-validation-scenarios-2026-07-25.md), OMN-63) closed for the enrollment scenarios (H1, H4, H10, H11) — can't sell managed enrollment on a mechanism not yet proven on real hardware |
| O2 Hardware bundles | Requires a documented recovery path per bundle (MISSION.md: "recovery paths are mandatory") — a customer who loses/breaks the bundled key must have a tested recovery flow before this ships, not after |
| O3 Support/SLA | Requires B-VM-CTAP2 closed (deterministic software-level FIDO2 coverage) — an SLA on a feature whose CI can't yet reliably prove itself is a support promise with no evidence backing it |
| O4 Managed CI | Requires PR #32 (sbsign + libykcs11 PKCS#11) merged and a documented "customer key stays customer-held" architecture, per covenant §2 |
| O5 Consulting | Requires the ARM64 fTPM stack status the consulting scope covers (see arm64-ftpm-phase-f0 / arm64-path-a-b-board-status refs) — don't scope a port to hardware whose own board rehearsal (B-ARM64-PATHA) hasn't run |
| O6 Training | No trust-chain readiness gate — gated only on training material existing and being accurate against current docs |
...
1. **O6 Training + O5 Consulting** — lowest readiness-gate dependency, can start
   once material/scoping exists; project-based, not recurring, so lowest
   commitment risk while other gates close.
2. **O3 Support/SLA** — recurring, but explicitly gated on B-VM-CTAP2 (already
   the leader's active CI mission per the pulpit) closing first.
3. **O1 Managed enrollment** — recurring, gated on the real-hardware validation
   scenarios (OMN-63, already drafted) actually being run, not just defined.
4. **O2 Hardware bundles** — non-recurring, gated on a recovery-path design that
   doesn't yet exist as a concrete doc.
5. **O4 Managed CI** — recurring but gated on PR #32 landing and a key-custody
   architecture that needs its own design work, not assumed here.
6. **O7 Grants/pilots** — external-funder timeline, gated on OMN-86, treated as
...
- That the ~$25–70 per-key hardware cost (MISSION.md) leaves enough margin at a
  bundle price point customers will actually pay, once pre-flash/enrollment
  labor is included — not validated by any pricing data fetched in this session.
- That a support SLA is sellable *before* B-VM-CTAP2 closes, or whether
  prospects wait for that evidence first — untested assumption in section 3.
